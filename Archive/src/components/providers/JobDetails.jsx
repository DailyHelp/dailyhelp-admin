...
export default function JobDetails({ usersData, jobs, onOpenChat }) {
  return (
      ...
      <div className="space-y-6 border-b border-[#D6DBE7] pb-4">
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex gap-3 items-center">
            <jobs.serviceProvider.icon className="rounded-full w-14 h-14" />
            {jobs.serviceProvider.badgeIcon && (
              <jobs.serviceProvider.badgeIcon className="absolute top-8 left-9" />
            )}
          ...
          <button
            ...
            className="p-[11px] rounded-2xl border border-[#D6DBE7] text-[#017441] text-sm font-bold"
          >
            View Chat
          </button>
        </div>
        ...
        <div className="space-y-1 border-b border-[#D6DBE7] py-4">
          ...
          {jobs.disputeDetails?.images && (
            <div className="flex gap-[2px] mt-2">
              <jobs.disputeDetails.images className="w-12 h-12" />
              <jobs.disputeDetails.images className="w-12 h-12" />
              <jobs.disputeDetails.images className="w-12 h-12" />
              <jobs.disputeDetails.images className="w-12 h-12" />
            </div>
          ...
          {jobs.resolutionDetails && (
            <div className="space-y- border-t border-[#D6DBE7] mt-4 pt-4">
              ...
              <p className="text-sm font-bold text-[#FF6B01] pt-3">
                {jobs.resolutionDetails.issue}
              </p>
              <p className="text-xl font-bold pb-1">₦{jobs.resolutionDetails.refundAmount}</p>
              <p className="text-[#3B4152] text-sm">{jobs.resolutionDetails.description}</p>
            </div>
          )}
        </div>
      )}
