...
export default function ChatView({ chat, jobs, usersData }) {
  return (
          ...
          return (
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`flex items-end gap-2 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                {/* Avatar */}
                {isUser ? (
                  <usersData.avatar className="rounded-full w-7 h-7" />
                ) : (
                  <jobs.serviceProvider.icon className="rounded-full w-7 h-7" />
                )}
