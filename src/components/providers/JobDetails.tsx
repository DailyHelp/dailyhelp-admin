...
import type { ProviderProfile, JobItem } from '@/types/types';
...
export interface ProviderJobDetailsProps {
  provider: ProviderProfile;
  job: JobItem;
  onOpenChat: (context: { providerUuid?: string; customerUuid?: string; job: JobItem }) => void;
}

export default function JobDetails({ provider, job, onOpenChat }: ProviderJobDetailsProps) {
  ...
  const timelineItems = useMemo<TimelineEntry[]>(() => {
    ...
    if (job.timeline?.accepted?.date) {
      manual.push({
        label: 'Offer Accepted',
        timestamp: `${job.timeline.accepted.date} ${job.timeline.accepted.time ?? ''}`.trim(),
      });
    }
    if (job.timeline?.started?.date) {
      manual.push({
        label: 'Job Start',
        timestamp: `${job.timeline.started.date} ${job.timeline.started.time ?? ''}`.trim(),
      });
    }
    if (job.timeline?.ended?.date) {
      manual.push({
        label: 'Job End',
        timestamp: `${job.timeline.ended.date} ${job.timeline.ended.time ?? ''}`.trim(),
      });
    }
    if (job.timeline?.resolution?.date) {
      manual.push({
        label: 'Resolution',
        timestamp: `${job.timeline.resolution.date} ${job.timeline.resolution.time ?? ''}`.trim(),
      });
    }
    return manual;
  }, [job.timeline, timelineResponse]);

  const dispute = disputeResponse?.data ?? null;
  const disputePictures = normalisePictures(dispute?.pictures);
  const amountDisplay = job.amount ? `₦${job.amount}` : '₦0';
  const acceptedDisplay = job.timeline?.accepted
    ? `${job.timeline.accepted.date} ${job.timeline.accepted.time ?? ''}`
    : '';

  const providerAvatar = job.serviceProvider?.icon as unknown;
  ...
  return (
    <section className="space-y-6 px-6 py-6">
      <header className="space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-lg font-semibold text-[#0E171A]">#{job.requestId ?? job.jobId}</p>
          </div>
        ...
        <div className="rounded-3xl border border-[#EAECF5] bg-white px-5 py-4">
          <div className="flex items-center gap-3">
            {typeof providerAvatar === 'string' && providerAvatar.trim().length > 0 ? (
              <Image
                src={providerAvatar}
                alt={job.serviceProvider?.name ?? ''}
                width={56}
                height={56}
                className="h-14 w-14 rounded-full object-cover"
              />
            ) : typeof providerAvatar === 'function' ? (
              (() => {
                const Comp = providerAvatar as React.ComponentType<React.SVGProps<SVGSVGElement>>;
                return <Comp className="h-14 w-14 rounded-full" />;
              })()
            ) : (
            ...
            <div>
              <p className="text-base font-semibold text-[#0E171A]">
                {job.serviceProvider?.name ?? provider.name}
              </p>
              <p className="text-sm font-medium uppercase tracking-wide text-[#99A1B3]">
                {job.serviceProvider?.role ?? provider.category ?? 'Service provider'}
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 space-y-1">
            ...
            <p className="text-sm text-[#47516B]">
              {job.jobDesc && job.jobDesc.trim().length > 0
                ? job.jobDesc
        ...
        <section className="space-y-4 rounded-3xl border border-[#FFE1D6] bg-[#FFFAFA] px-5 py-4">
          <div className="space-y-1">
            ...
            <p className="text-sm font-semibold text-[#0E171A]">
              {dispute.category ?? 'Dispute reported'}
            </p>
            {dispute.description ? (
      ...
      <section className="space-y-4">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-[#99A1B3]">
          Job timeline
        </h3>
          ...
          <ul className="relative space-y-4">
            {timelineItems.map((item, index) => (
              <li
                ...
                className="relative flex items-center justify-between gap-4 pl-8"
              >
                ...
                <span className="text-xs font-medium text-[#99A1B3]">
