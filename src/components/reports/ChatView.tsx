...
export interface ReportsChatViewProps {
  job: ReportEntry;
  providerUuid?: string;
  customerUuid?: string;
}
...
function ChatBubble({
    ...
    return (
      ...
      >
        <div className={`rounded-3xl px-4 py-4 ${meta.wrapperClass}`}>
          <div className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ${meta.badgeClass}`}>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-2 text-xs font-semibold">
              {isCustomerMessage ? (
                ...
                <div className="flex gap-2">
                  <button className="rounded-full bg-[#0D8941] px-4 py-2 text-white">
                    Accept
                  </button>
  ...
  return (
    <div className={`space-y-2 rounded-3xl px-5 py-4 text-sm leading-6 shadow-[0_12px_40px_rgba(18,25,33,0.06)] ${bubbleClass}`}>
...
export default function ChatView({ job, providerUuid, customerUuid }: ReportsChatViewProps) {
  ...
  const groupedMessages = useMemo(() => {
    ...
    const groups = new Map<number, { label: string; items: typeof messages }>();
  ...
  if (!providerUuid || !customerUuid) {
    return (
      <div className="flex h-full items-center justify-center bg-[#F4F5FA] px-6 text-center text-sm text-[#757C91]">
        Conversation history is unavailable because we are missing the participant information.
      </div>
    );
  }
  ...
  const customerAvatar = job.client?.icon ?? job.reporter?.icon;
  const providerAvatar = job.serviceProvider?.icon ?? job.reportedParty?.icon;
  const customerInitial = job.client?.name?.[0]?.toUpperCase() ?? job.reporter?.name?.[0]?.toUpperCase() ?? 'C';
  const providerInitial = job.serviceProvider?.name?.[0]?.toUpperCase() ?? job.reportedParty?.name?.[0]?.toUpperCase() ?? 'P';
      ...
      >
        ...
        {groupedMessages.length === 0 ? (
          <p className="text-center text-sm text-[#757C91]">
            No conversation history available for this report yet.
          </p>
        ) : (
          groupedMessages.map((group) => (
            <div key={group.key} className="space-y-4">
              ...
              {group.items.map((message) => {
                ...
                const alignmentClass = isCustomerMessage ? 'items-end text-right' : 'items-start text-left';
                const attachments = normalisePictures(
                  message.attachments as string | string[] | null | undefined,
                );
                const isOffer = message.type?.toUpperCase() === 'OFFER';
                const amountDisplay = formatCurrency(message.amount ?? null);

                return (
                  <div key={message.uuid} className={`flex flex-col gap-2 ${alignmentClass}`}>
