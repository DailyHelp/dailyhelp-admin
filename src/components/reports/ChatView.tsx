...
export default function ChatView({ chat, job }: { chat: any[]; job: any }) {
  return (
    <div className="h-[100vh] bg-[#F1F2F4] overflow-hidden">
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {chat[0].messages.map((msg: any) => {
          ...
          return (
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex items-end gap-2 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                {isUser
                  ? (() => {
                      const Icon = job.reporter?.icon as unknown as React.ComponentType<
                        React.SVGProps<SVGSVGElement>
                      >;
                      return Icon ? <Icon className="rounded-full w-7 h-7" /> : null;
                    })()
                  : (() => {
                      const Icon = job.reportedParty?.icon as unknown as React.ComponentType<
                        React.SVGProps<SVGSVGElement>
                      >;
                      return Icon ? <Icon className="rounded-full w-7 h-7" /> : null;
                    })()}
                    ...
                    >
                      ...
                      {msg.images && msg.images.length > 0 && (
                        <div className="flex gap-[2px] mt-2">
                          {msg.images.map((src: string, idx: number) => (
