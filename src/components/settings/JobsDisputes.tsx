...
export default function JobsDisputes({
  ...
  handleDeleteClick?: (payload: any) => void;
}) {
  const items = useMemo<SettingsCategoryItem[]>(() => report ?? [], [report]);
  const [serviceProvider, setServiceProvider] = useState<string>('');
  const [client, setClient] = useState<string>('');
  const [autoBlock, setAutoBlock] = useState<Record<string, boolean>>({}); // per jobId#index
  ...
  const keyFor = (jobId: string, index: number) => `${jobId}#${index}`;
  ...
  const handleToggle = (jobId: string, index: number) => {
