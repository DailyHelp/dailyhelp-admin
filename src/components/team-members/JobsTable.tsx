...
import { TeamMember } from '@/types/types'; // or wherever you defined TeamMember
import Button from '@/components/ui/Button';

interface JobsTableProps {
  jobs: TeamMember[];
  sortConfig: { key: keyof TeamMember | null; direction: 'asc' | 'desc' };
  onSortChange: (config: { key: keyof TeamMember; direction: 'asc' | 'desc' }) => void;
  onOpenJobDetails: (job: TeamMember) => void;
  handleSuspendClick: (user: TeamMember) => void;
...
export default function JobsTable({
  ...
  const getSortIcon = (columnKey: keyof TeamMember | 'emailaddress') => {
  ...
  const handleSort = (columnKey: keyof TeamMember) => {
    let direction: 'asc' | 'desc' = 'asc'; // ✅ explicitly typed
  ...
  return (
    <div className="mx-6 overflow-x-auto">
      <table className="w-full min-w-[720px] text-left border-collapse" role="table" aria-label="Team members table">
